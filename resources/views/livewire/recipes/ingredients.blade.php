        ...
        @if (count($ingredients))
            @while (($ingredient = current($ingredients)) !== false)
                @php
                    ...
                    if (($p = strrpos($unit, ']')) !== false) {
                        $unit = substr($unit, $p + 1);
                    }
                ...
                <x-table.row wire:loading.class.delay="opacity-50">
                    ...
                    <x-table.cell>{{ calculate_unit($ingredient->ingredient?->name,1) }}{{ is_null($ingredient->ingredient?->info) ? '' : ' (' . $ingredient->ingredient?->info . ')' }}</x-table.cell>
